// -----------------------------------------------------------------------------
// gps_simulator.cpp
// NAV-PVT simulator (fixed 5 Hz tick)
//
// - Generates realistic sailing motion
// - Simulates satellite acquisition
// - Provides stable, advancing UTC time for filenames & logging
// -----------------------------------------------------------------------------
...
static constexpr float
  STRAIGHT_DIST_M = 200.0f,
  KNOTS_TO_MPS   = 0.514444f,
  STRAIGHT_KTS   = 40.0f,
  TURN_KTS       = 20.0f;
...
// Simulator state
// -----------------------------------------------------------------------------
static uint32_t last_emit_ms=0, sim_ms=0, last_sat_step=0;
...
static inline void ramp_speed(double target,double dt){
  ...
  speed_mps += (fabs(diff) <= max_delta) ? diff : (diff > 0 ? max_delta : -max_delta);
}
...
void gps_simulator_init()
{
  ...
  sim_ms=0; last_emit_ms=millis(); last_sat_step=last_emit_ms;
  sat_count=0;
...
int gps_simulator_step()
{
  ...
  if(!time_init && ubxMessage.navPvt.fixType >= 3){
    ...
    ubxMessage.navPvt.sec   = (sim_ms/1000) % 60;
    ubxMessage.navPvt.nano  = 0;
    ubxMessage.navPvt.tAcc  = 50000;
    ubxMessage.navPvt.valid = 0b111;   // date + time + fully resolved
  ...
  if(time_init){
    ...
    if(sec_tick != last_sec_tick){
      const uint32_t delta = sec_tick - last_sec_tick;
      last_sec_tick = sec_tick;

      uint32_t s = ubxMessage.navPvt.sec + delta;
      ubxMessage.navPvt.sec = s % 60;
      // minutes/hours rollover not required for logging
    }
  }
  ...
  // ---------------------------------------------------------------------------
  if(!turning){
    ramp_speed(STRAIGHT_KTS*KNOTS_TO_MPS, dt);
    ...
    if(leg_distance >= STRAIGHT_DIST_M){
      ...
      turn_progress_deg=0.0;
      turn_rate_deg_per_sec=(speed_mps/turn_radius)*(180.0/M_PI);
    }
  }else{
    ramp_speed(TURN_KTS*KNOTS_TO_MPS, dt);
    speed_mps += randf(-0.10,0.10) * KNOTS_TO_MPS;

    const double dh = turn_rate_deg_per_sec * dt;
    heading_deg += dh; turn_progress_deg += fabs(dh);
