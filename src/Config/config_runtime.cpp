...
int timezoneKey(float timezone)
{
  return static_cast<int>(timezone * 100);
}

void setStandardTimezone(float timezone)
{
  const int hours = static_cast<int>(timezone);
  const int minutes = abs(static_cast<int>(timezone * 60) % 60);
...
void applyDstTimezone(float timezone)
{
  if (!config.timezone_DST) return;

  switch (timezoneKey(timezone)) {
