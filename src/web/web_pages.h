...
footer{position:sticky;bottom:0;z-index:500;padding:16px;border-top:1px solid var(--e);background:#f0f4fa}
...
function markDirty(){if(!dirty){dirty=true;saveBtn.disabled=false}}

document.addEventListener("input",e=>{if(e.target&&!e.target._loading)markDirty()});
