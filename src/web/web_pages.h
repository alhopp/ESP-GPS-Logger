...
.modal-card ul{padding-left:18px}.modal-card .note{margin-top:10px;font-size:13px;color:var(--f)}
.modal-card button{margin-top:14px;width:100%;padding:12px;border:0;border-radius:10px;background:var(--a);color:#fff;font-size:16px}
</style></head><body>
...
<section id=files class=a><div class=card>
<h3>Logs</h3><div id=sdInfo>Scanning…</div>
...
<div class=card>
<h3>Screens<span class=info onclick="openInfo()">ⓘ</span></h3>
...
<option value=0>Off</option><option value=1>Generic</option><option value=2>Slalom</option>
<option value=3>Foil</option><option value=4>Wave</option><option value=5>Speed</option>
</select>
<label>Sail Logo</label>
<select id=Sail_Logo>
<option value=0>Off</option><option value=1>Neutral</option>
<option value=2>Race</option><option value=3>Wave</option><option value=4>Foil</option>
</select>
</div>
...
<p>These screens control which performance statistics can be displayed on the device.</p>
<ul>
...
<p class=note>These options are informational only and do not affect logging yet.</p>
...
async function save(){
const p={ui:{Sleep_info:Sleep_info.value,Stat_screens:+Stat_screens.value,stat_screen:stat_screen.value,Board_Logo:+Board_Logo.value,Sail_Logo:+Sail_Logo.value},
...
j.files.forEach(f=>fileList.innerHTML+=`<div class=file-row><div><div class=file-name>${f.name}</div><div class=file-size>${(f.size/1024).toFixed(1)} KB</div></div></div>`)}
load();
</script>
</body></html>
)rawliteral";
